...
int test_matrixCompMult() {
...

  {
    ...
    mat4 expected =
        mat4(0, 1, 4, 9, 16, 25, 36, 49, 64, 81, 100, 121, 144, 169, 196, 225);
    Error += all(equal(n, expected, epsilon<float>())) ? 0 : 1;
  }
...
int test_inverse() {
  ...
  {
    glm::mat4x4 A4x4(glm::vec4(1, 0, 1, 0), glm::vec4(0, 1, 0, 0),
  ...
  {
    glm::mat3x3 A3x3(glm::vec3(1, 0, 1), glm::vec3(0, 1, 0),
                     glm::vec3(0, 0, 1));
...
int test_inverse_simd() {
  ...
  glm::mat4x4 const A4x4(glm::vec4(1, 0, 1, 0), glm::vec4(0, 1, 0, 0),
                         glm::vec4(0, 0, 1, 0), glm::vec4(0, 0, 0, 1));
...
int test_inverse_perf(std::size_t Count, std::size_t Instance,
                      char const* Message) {
  ...
  for (std::size_t i = 0; i < TestInputs.size(); ++i) {
    typename MAT4::value_type f =
        static_cast<typename MAT4::value_type>(i + Instance) *
  ...
  std::printf("inverse<%s>(%f): %lu\n", Message, static_cast<double>(Diff),
              EndTime - StartTime);
