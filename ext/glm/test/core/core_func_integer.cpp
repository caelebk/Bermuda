...
static int test() {
  ...
  for (glm::uint i = 0; i < count; ++i) {
    glm::uint Return = glm::bitfieldInsert(Data32[i].Base, Data32[i].Insert,
                                           Data32[i].Offset, Data32[i].Bits);
...
static int test() {
  ...
  for (glm::uint i = 0; i < count; ++i) {
    glm::uint Return =
        glm::bitfieldExtract(Data32[i].Value, Data32[i].Offset, Data32[i].Bits);
...
/*
        GLM_FUNC_QUALIFIER unsigned int bitfieldReverseLoop(unsigned int v)
        {
                ...
                unsigned int const BitSize = static_cast<unsigned
   int>(sizeof(unsigned int) * 8); for(unsigned int i = 0; i < BitSize; ++i)
                {
                        unsigned int const BitSet(v & (static_cast<unsigned
   int>(1) << i)); unsigned int const BitFirst(BitSet >> i); Result |= BitFirst
   << (BitSize - 1 - i);
                }
                return Result;
        }
        ...
        GLM_FUNC_QUALIFIER glm::uint64_t bitfieldReverseLoop(glm::uint64_t v)
        {
                ...
                glm::uint64_t const BitSize =
   static_cast<glm::uint64_t>(sizeof(unsigned int) * 8); for(glm::uint64_t i =
   0; i < BitSize; ++i)
                {
                        glm::uint64_t const BitSet(v &
   (static_cast<glm::uint64_t>(1) << i)); glm::uint64_t const BitFirst(BitSet >>
   i); Result |= BitFirst << (BitSize - 1 - i);
                }
                return Result;
        }
*/
template <glm::length_t L, typename T, glm::qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> bitfieldReverseLoop(
...
struct compute_bitfieldReverseStep {
  template <glm::length_t L, typename T, glm::qualifier Q>
  GLM_FUNC_QUALIFIER static glm::vec<L, T, Q> call(glm::vec<L, T, Q> const& v,
                                                   T, T) {
    return v;
  }
};
...
template <glm::length_t L, typename T, glm::qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> bitfieldReverseOps(
    glm::vec<L, T, Q> const& v) {
  glm::vec<L, T, Q> x(v);
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 2>::call(
      x, static_cast<T>(0x5555555555555555ull), static_cast<T>(1));
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 4>::call(
      x, static_cast<T>(0x3333333333333333ull), static_cast<T>(2));
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 8>::call(
      x, static_cast<T>(0x0F0F0F0F0F0F0F0Full), static_cast<T>(4));
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 16>::call(
      x, static_cast<T>(0x00FF00FF00FF00FFull), static_cast<T>(8));
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 32>::call(
      x, static_cast<T>(0x0000FFFF0000FFFFull), static_cast<T>(16));
  x = compute_bitfieldReverseStep<sizeof(T) * 8 >= 64>::call(
...
typeU32 const Data32[] = {
    {0x00000001, 0x80000000, SUCCESS}, {0x0000000f, 0xf0000000, SUCCESS},
...
static int perf_int(std::size_t Count) {
  type<int, int> const Data[] = {
      {0x00000000, -1}, {0x00000001, 0},  {0x00000002, 1},  {0x00000003, 1},
      {0x00000004, 2},  {0x00000005, 2},  {0x00000007, 2},  {0x00000008, 3},
      ...
      {0x10000000, 28}, {0x20000000, 29}, {0x40000000, 30}};
...
static int test_ivec4() {
  ...
  for (std::size_t i = 0;
       i < sizeof(Data) / sizeof(type<glm::ivec4, glm::ivec4>); ++i) {
...
static int findLSB_branchfree(genIUType x) {
  ...
  return static_cast<int>(glm::bitCount(~x & (x - static_cast<genIUType>(1)))) *
             Keep +
         Discard * -1;
}
...
static int test() {
...

  {
    ...
    Error += glm::all(glm::equal(
                 Result, glm::uvec1(std::numeric_limits<glm::uint>::max())))
                 ? 0
                 : 1;
  }
...
type<int> const DataI32[] = {{0x00000001, 1},
                             {0x00000003, 2},
                             {0x00000002, 1},
                             {0x7fffffff, 31},
                             {0x00000000, 0}};
...
static glm::vec<L, int, Q> bitCount_bitfield(glm::vec<L, T, Q> const& v) {
  glm::vec<L, typename glm::detail::make_unsigned<T>::type, Q> x(
      *reinterpret_cast<
          glm::vec<L, typename glm::detail::make_unsigned<T>::type, Q> const*>(
          &v));
  x = compute_bitfieldBitCountStep<sizeof(T) * 8 >= 2>::call(
      x,
      static_cast<typename glm::detail::make_unsigned<T>::type>(
