...
static int test_operators() {
...

  {
    ...
    A += 1.0f;
    Error +=
        glm::all(glm::equal(A, glm::vec2(2.0f), glm::epsilon<float>())) ? 0 : 1;
...

  {
    ...
    A += B;
    Error +=
        glm::all(glm::equal(A, glm::vec2(3.0f, 0.0f), glm::epsilon<float>()))
            ? 0
            : 1;
    Error += glm::all(glm::equal(A, C, glm::epsilon<float>())) ? 0 : 1;
  }

  {
    ...
    A -= 1.0f;
    Error +=
        glm::all(glm::equal(A, glm::vec2(0.0f), glm::epsilon<float>())) ? 0 : 1;
    Error += glm::all(glm::equal(A, C, glm::epsilon<float>())) ? 0 : 1;
  }

  {
    ...
    A -= B;
    Error +=
        glm::all(glm::equal(A, glm::vec2(-1.0f, 2.0f), glm::epsilon<float>()))
            ? 0
            : 1;
    Error += glm::all(glm::equal(A, C, glm::epsilon<float>())) ? 0 : 1;
  }
...

  {
    ...
    A /= B;
    Error +=
        glm::all(glm::equal(A, glm::vec2(1.0f), glm::epsilon<float>())) ? 0 : 1;
    Error += glm::all(glm::equal(A, C, glm::epsilon<float>())) ? 0 : 1;
  }

  {
    ...
    glm::vec2 C = A + B;
    Error +=
        glm::all(glm::equal(C, glm::vec2(5, 7), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 D = B - A;
    Error +=
        glm::all(glm::equal(D, glm::vec2(3, 3), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 E = A * B;
    Error += glm::all(glm::equal(E, glm::vec2(4, 10), glm::epsilon<float>()))
                 ? 0
                 : 1;

    glm::vec2 F = B / A;
    Error += glm::all(glm::equal(F, glm::vec2(4, 2.5), glm::epsilon<float>()))
                 ? 0
                 : 1;

    glm::vec2 G = A + 1.0f;
    Error +=
        glm::all(glm::equal(G, glm::vec2(2, 3), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 H = B - 1.0f;
    Error +=
        glm::all(glm::equal(H, glm::vec2(3, 4), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 I = A * 2.0f;
    Error +=
        glm::all(glm::equal(I, glm::vec2(2, 4), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 J = B / 2.0f;
    Error += glm::all(glm::equal(J, glm::vec2(2, 2.5), glm::epsilon<float>()))
                 ? 0
                 : 1;

    glm::vec2 K = 1.0f + A;
    Error +=
        glm::all(glm::equal(K, glm::vec2(2, 3), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 L = 1.0f - B;
    Error += glm::all(glm::equal(L, glm::vec2(-3, -4), glm::epsilon<float>()))
                 ? 0
                 : 1;

    glm::vec2 M = 2.0f * A;
    Error +=
        glm::all(glm::equal(M, glm::vec2(2, 4), glm::epsilon<float>())) ? 0 : 1;

    glm::vec2 N = 2.0f / B;
    Error += glm::all(glm::equal(N, glm::vec2(0.5, 2.0 / 5.0),
                                 glm::epsilon<float>()))
                 ? 0
                 : 1;
  }

  {
    ...
    A += B;
    Error +=
        glm::all(glm::equal(A, glm::vec2(5, 7), glm::epsilon<float>())) ? 0 : 1;

    A += 1.0f;
    Error +=
        glm::all(glm::equal(A, glm::vec2(6, 8), glm::epsilon<float>())) ? 0 : 1;
  }
  {
...
static int test_ctor() {
...
#if GLM_HAS_TRIVIAL_QUERIES
  //	Error += std::is_trivially_default_constructible<glm::vec2>::value ? 0 :
  //1; 	Error += std::is_trivially_copy_assignable<glm::vec2>::value ? 0 : 1;
...

  {
    ...
    // glm::vec2 D = glm::dvec2(2.0); // Build error TODO: What does the
    // specification says?
...
#endif  // GLM_CONFIG_SWIZZLE == GLM_SWIZZLE_OPERATOR || GLM_CONFIG_SWIZZLE ==
