...
static int test_equal() {
  ...
  Error += glm::all(glm::equal(matType(One), matType(Two), vecType(Epsilon)))
               ? 1
               : 0;
...
static int test_notEqual() {
  ...
  Error +=
      !glm::any(glm::notEqual(matType(One), matType(One), Epsilon)) ? 0 : 1;
  Error +=
      !glm::any(glm::notEqual(matType(One), matType(Two), vecType(Epsilon)))
          ? 1
          : 0;
