...
int test_int16() {
  type<glm::int16> const Data[] = {{0x0001, true}, {0x0002, true},
                                   {0x0004, true}, {0x0080, true},
                                   {0x0000, true}, {0x0003, false}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::int16>); i < n;
       ++i) {
    glm::vec<L, bool> const Result =
        glm::isPowerOfTwo(glm::vec<L, glm::int16>(Data[i].Value));
...
template <glm::length_t L>
int test_uint16() {
  type<glm::uint16> const Data[] = {{0x0001, true}, {0x0002, true},
                                    {0x0004, true}, {0x0000, true},
                                    {0x0000, true}, {0x0003, false}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::uint16>); i < n;
       ++i) {
    glm::vec<L, bool> const Result =
        glm::isPowerOfTwo(glm::vec<L, glm::uint16>(Data[i].Value));
    Error += glm::vec<L, bool>(Data[i].Return) == Result ? 0 : 1;
  }
...
template <glm::length_t L>
int test_int32() {
  type<int> const Data[] = {{0x00000001, true}, {0x00000002, true},
                            {0x00000004, true}, {0x0000000f, false},
                            {0x00000000, true}, {0x00000003, false}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<int>); i < n; ++i) {
    glm::vec<L, bool> const Result =
        glm::isPowerOfTwo(glm::vec<L, glm::int32>(Data[i].Value));
    Error += glm::vec<L, bool>(Data[i].Return) == Result ? 0 : 1;
  }
...
template <glm::length_t L>
int test_uint32() {
  type<glm::uint> const Data[] = {{0x00000001, true}, {0x00000002, true},
                                  {0x00000004, true}, {0x80000000, true},
                                  {0x00000000, true}, {0x00000003, false}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::uint>); i < n;
       ++i) {
    glm::vec<L, bool> const Result =
        glm::isPowerOfTwo(glm::vec<L, glm::uint32>(Data[i].Value));
    Error += glm::vec<L, bool>(Data[i].Return) == Result ? 0 : 1;
  }
...
int run() {
  ...
  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
    glm::vec<L, T> const Result0 =
        glm::prevMultiple(glm::vec<L, T>(Data[i].Source), Data[i].Multiple);
    Error += glm::vec<L, T>(Data[i].Return) == Result0 ? 0 : 1;

    glm::vec<L, T> const Result1 = glm::prevMultiple(
        glm::vec<L, T>(Data[i].Source), glm::vec<L, T>(Data[i].Multiple));
...
template <glm::length_t L, typename T>
int run() {
  type<T> const Data[] = {{3, 4, 4}, {6, 3, 6}, {5, 3, 6},
                          {7, 7, 7}, {0, 1, 0}, {8, 3, 9}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
    glm::vec<L, T> const Result0 = glm::nextMultiple(
        glm::vec<L, T>(Data[i].Source), glm::vec<L, T>(Data[i].Multiple));
    Error += glm::vec<L, T>(Data[i].Return) == Result0 ? 0 : 1;

    glm::vec<L, T> const Result1 =
        glm::nextMultiple(glm::vec<L, T>(Data[i].Source), Data[i].Multiple);
    Error += glm::vec<L, T>(Data[i].Return) == Result1 ? 0 : 1;
  }
...
int run() {
  type<T> const Data[] = {
      {0x00, 1, -1}, {0x01, 2, -1}, {0x02, 2, -1}, {0x06, 3, -1}, {0x01, 1, 0},
      {0x03, 1, 0},  {0x03, 2, 1},  {0x07, 2, 1},  {0x05, 2, 2},  {0x0D, 2, 2}};

  int Error = 0;

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
    glm::vec<L, int> const Result0 = glm::findNSB<L, T, glm::defaultp>(
        glm::vec<L, T>(Data[i].Source),
        glm::vec<L, int>(Data[i].SignificantBitCount));
