...
static int test_translate() {
  ...
  Error += glm::all(glm::equal(T[3], glm::vec4(1.0f), glm::epsilon<float>()))
               ? 0
               : 1;
...
static int test_scale() {
  ...
  glm::mat4 const R = glm::mat4(glm::vec4(2, 0, 0, 0), glm::vec4(0, 2, 0, 0),
...
static int test_rotate() {
  ...
  Error += glm::all(glm::equal(B, glm::vec4(0.0f, 1.0f, 0.0f, 1.0f), 0.0001f))
               ? 0
               : 1;
