...
static int test_operators() {
...

  {
    ...
    genType const C = A + B;
    Error +=
        glm::all(glm::equal(C, genType(2), glm::epsilon<valType>())) ? 0 : 1;

    genType const D = A - B;
    Error +=
        glm::all(glm::equal(D, genType(0), glm::epsilon<valType>())) ? 0 : 1;

    genType const E = A * B;
    Error +=
        glm::all(glm::equal(E, genType(1), glm::epsilon<valType>())) ? 0 : 1;

    genType const F = A / B;
    Error +=
        glm::all(glm::equal(F, genType(1), glm::epsilon<valType>())) ? 0 : 1;
  }
