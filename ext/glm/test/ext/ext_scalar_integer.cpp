...
int test_int32() {
  ...
  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::int32>); i < n;
       ++i) {
...
int test_uint32() {
  type<glm::uint32> const Data[] = {
      ...
      {0x00000004, 0x00000004}, {0x00000007, 0x00000008},
      {0x0000ffff, 0x00010000}, {0x0000fff0, 0x00010000},
      {0x0000f000, 0x00010000}, {0x80000000, 0x80000000},
      {0x00000000, 0x00000000}, {0x00000003, 0x00000004}};

  int Error(0);

  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::uint32>); i < n;
       ++i) {
...
int perf_nextMultiple(glm::uint Samples) {
  ...
  for (glm::uint Source = 0; Source < Samples; ++Source)
    for (glm::uint Multiple = 0; Multiple < Multiples; ++Multiple) {
      Results[Source * Multiples + Multiple] =
          glm::nextMultiple(Source, Multiples);
    }
  ...
  glm::uint Result = 0;
  for (std::size_t i = 0, n = Results.size(); i < n; ++i) Result += Results[i];
...
int perf_nextMultipleMod(glm::uint Samples) {
  ...
  for (glm::uint Multiple = 0; Multiple < Multiples; ++Multiple)
    for (glm::uint Source = 0; Source < Samples; ++Source) {
      Results[Source * Multiples + Multiple] =
          nextMultipleMod(Source, Multiples);
    }
...
int perf_nextMultipleNeg(glm::uint Samples) {
  ...
  for (glm::uint Source = 0; Source < Samples; ++Source)
    for (glm::uint Multiple = 0; Multiple < Multiples; ++Multiple) {
      Results[Source * Multiples + Multiple] =
          nextMultipleNeg(Source, Multiples);
    }
...
int perf_nextMultipleUFloat(glm::uint Samples) {
  ...
  for (glm::uint Source = 0; Source < Samples; ++Source)
    for (glm::uint Multiple = 0; Multiple < Multiples; ++Multiple) {
      Results[Source * Multiples + Multiple] = nextMultipleUFloat(
          static_cast<float>(Source), static_cast<float>(Multiples));
    }
...
int perf_nextMultipleFloat(glm::uint Samples) {
  ...
  for (glm::uint Source = 0; Source < Samples; ++Source)
    for (glm::uint Multiple = 0; Multiple < Multiples; ++Multiple) {
      Results[Source * Multiples + Multiple] = nextMultipleFloat(
          static_cast<float>(Source), static_cast<float>(Multiples));
    }
...
int run() {
  ...
  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<T>); i < n; ++i) {
    int const Result0 =
        glm::findNSB(Data[i].Source, Data[i].SignificantBitCount);
    Error += Data[i].Return == Result0 ? 0 : 1;
    assert(!Error);
  }
