...
glm::vec4 const AngleShift(0.0f, glm::half_pi<float>(), glm::pi<float>(),
                           glm::three_over_two_pi<float>());

template <length_t L, typename T, qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> taylorSeriesNewCos(
    glm::vec<L, T, Q> const& x) {
...
template <length_t L, typename T, qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> taylorSeriesNewCos6(
    glm::vec<L, T, Q> const& x) {
  glm::vec<L, T, Q> const Powed2(x * x);
  glm::vec<L, T, Q> const Powed4(Powed2 * Powed2);
  glm::vec<L, T, Q> const Powed6(Powed4 * Powed2);
...
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> fastCosNew(glm::vec<L, T, Q> const& x) {
  glm::vec<L, T, Q> const Angle0_PI(
      fastAbs(fmod(x + glm::pi<T>(), glm::two_pi<T>()) - glm::pi<T>()));
  return taylorSeriesNewCos6(x);
  /*
                  vec<L, bool, Q> const FirstQuarterPi(lessThanEqual(Angle0_PI,
     vec<L, T, Q>(glm::half_pi<T>())));

                  vec<L, T, Q> const RevertAngle(mix(vec<L, T, Q>(glm::pi<T>()),
     vec<L, T, Q>(0), FirstQuarterPi)); vec<L, T, Q> const ReturnSign(mix(vec<L,
     T, Q>(-1), vec<L, T, Q>(1), FirstQuarterPi)); vec<L, T, Q> const
     SectionAngle(RevertAngle - Angle0_PI);
...
int perf_fastCosNew(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] = fastCosNew(AngleShift +
                            glm::vec4(Begin + Steps * static_cast<float>(i)));
...
template <length_t L, typename T, qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> deterministic_fmod(
    glm::vec<L, T, Q> const& x, T y) {
  return x - y * trunc(x / y);
}

template <length_t L, typename T, qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> fastCosDeterminisctic(
    glm::vec<L, T, Q> const& x) {
  glm::vec<L, T, Q> const    Angle0_PI(abs(
         deterministic_fmod(x + glm::pi<T>(), glm::two_pi<T>()) - glm::pi<T>()));
  glm::vec<L, bool, Q> const FirstQuarterPi(
      lessThanEqual(Angle0_PI, glm::vec<L, T, Q>(glm::half_pi<T>())));

  glm::vec<L, T, Q> const RevertAngle(mix(
      glm::vec<L, T, Q>(glm::pi<T>()), glm::vec<L, T, Q>(0), FirstQuarterPi));
  glm::vec<L, T, Q> const ReturnSign(
      mix(glm::vec<L, T, Q>(-1), glm::vec<L, T, Q>(1), FirstQuarterPi));
...
template <length_t L, typename T, qualifier Q>
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> taylorSeriesRefCos(
    glm::vec<L, T, Q> const& x) {
...
GLM_FUNC_QUALIFIER glm::vec<L, T, Q> fastRefCos(glm::vec<L, T, Q> const& x) {
  glm::vec<L, T, Q> const Angle0_PI(
      glm::abs(fmod(x + glm::pi<T>(), glm::two_pi<T>()) - glm::pi<T>()));
...
int perf_fastCosRef(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] = taylorCos::fastRefCos(
        AngleShift + glm::vec4(Begin + Steps * static_cast<float>(i)));
...
int perf_fastCosOld(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] = glm::fastCos(AngleShift +
                              glm::vec4(Begin + Steps * static_cast<float>(i)));
...
int perf_cos(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] =
        glm::cos(AngleShift + glm::vec4(Begin + Steps * static_cast<float>(i)));
...
int test() {
  ...
  // for(float Angle = -4.0f * glm::pi<float>(); Angle < 4.0f *
  // glm::pi<float>(); Angle += 0.1f) for(float Angle = -720.0f; Angle < 720.0f;
  // Angle += 0.1f)
...
namespace taylor2 {
glm::vec4 const AngleShift(0.0f, glm::pi<float>() * 0.5f,
                           glm::pi<float>() * 1.0f, glm::pi<float>() * 1.5f);

float taylorCosA(float x) {
  return 1.f - (x * x) * (1.f / 2.f) + (x * x * x * x) * (1.f / 24.f) -
...
int perf_taylorCosA(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] =
        taylorCosA(AngleShift.x + Begin + Steps * static_cast<float>(i));
...
int perf_taylorCosB(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] =
        taylorCosB(AngleShift.x + Begin + Steps * static_cast<float>(i));
...
int perf_taylorCosC(float Begin, float End, std::size_t Samples) {
  ...
  for (std::size_t i = 0; i < Samples; ++i)
    Results[i] =
        taylorCosC(AngleShift.x + Begin + Steps * static_cast<float>(i));
