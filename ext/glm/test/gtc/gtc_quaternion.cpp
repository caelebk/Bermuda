...
int test_quat_angle() {
  ...
  {
    glm::quat Q = glm::angleAxis(glm::pi<float>() * 0.25f,
...
int test_quat_euler() {
...

  {
    ...
    Error +=
        glm::all(glm::equal(Angles, glm::vec3(Pitch, Yaw, Roll), 0.000001f))
            ? 0
            : 1;
  }

  {
    ...
    Error +=
        glm::all(glm::equal(Angles, glm::dvec3(Pitch, Yaw, Roll), 0.000001))
            ? 0
            : 1;
  }
...
int test_quat_slerp() {
  ...
  glm::quat id(static_cast<float>(1), static_cast<float>(0),
  ...
  // Testing almost equal quaternions (this test should pass through the linear
  // interpolation) Must be 0 0.00X 0 0.99999
  glm::quat almostid =
      glm::slerp(id, glm::angleAxis(0.1f, glm::vec3(0.0f, 1.0f, 0.0f)), 0.5f);
  ...
  // Testing quaternions with opposite sign
  {
    ...
    Error +=
        glm::equal(glm::pow(glm::dot(id, result), 2.f), 1.f, 0.01f) ? 0 : 1;
  }
...
static int test_mul() {
  ...
  glm::vec3 transformed0 =
      (temp1 * glm::vec3(0.0, 1.0, 0.0) * glm::inverse(temp1));
  ...
  Error +=
      glm::any(glm::notEqual(temp7, glm::quat(1.0f, glm::vec3(0.0, 1.0, 0.0)),
                             glm::epsilon<float>()))
          ? 1
          : 0;
