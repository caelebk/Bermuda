...
inline int mask_mix(int Bits) {
  return Bits >= sizeof(int) * 8
             ? 0xffffffff
             : (static_cast<int>(1) << Bits) - static_cast<int>(1);
}
...
int test_uint() {
  type<glm::uint> const Data[] = {{0, 0x00000000},  {1, 0x00000001},
                                  {2, 0x00000003},  {3, 0x00000007},
                                  {31, 0x7fffffff}, {32, 0xffffffff}};

  int Error = 0;
  /* mask_zero is sadly not a correct code
                  for(std::size_t i = 0; i < sizeof(Data) / sizeof(type<int>);
     ++i)
                  {
...
int test_uvec4() {
  type<glm::ivec4> const Data[] = {{glm::ivec4(0), glm::ivec4(0x00000000)},
  ...
  for (std::size_t i = 0, n = sizeof(Data) / sizeof(type<glm::ivec4>); i < n;
       ++i) {
  ...
  for (glm::uint16 z = 0; z < z_max; z += 27)
    for (glm::uint16 y = 0; y < y_max; y += 27)
      for (glm::uint16 x = 0; x < x_max; x += 27) {
        glm::uint64 ResultA =
            refBitfieldInterleave<glm::uint16, glm::uint64>(x, y, z);
  ...
  for (glm::uint16 w = 0; w < w_max; w += 27)
    for (glm::uint16 z = 0; z < z_max; z += 27)
      for (glm::uint16 y = 0; y < y_max; y += 27)
        for (glm::uint16 x = 0; x < x_max; x += 27) {
          glm::uint64 ResultA =
              loopBitfieldInterleave<glm::uint16, glm::uint64>(x, y, z, w);
...
inline glm::uint64 sseUnalignedBitfieldInterleave(glm::uint32 x,
                                                  glm::uint32 y) {
  __m128i const Array = _mm_set_epi32(0, y, 0, x);
...
int test() {
  ...
  /*
                  {
                          ...
                          for(glm::uint32 x = 0; x < (1 << 10); ++x)
                          {
                                  ...
                                  //glm::uint64 C = loopBitfieldInterleave(x,
  y); glm::uint64 D = interleaveBitfieldInterleave(x, y);
  ...
  #				if GLM_ARCH & GLM_ARCH_SSE2_BIT
                                          glm::uint64 E =
  sseBitfieldInterleave(x, y); glm::uint64 F = sseUnalignedBitfieldInterleave(x,
  y); assert(A == E); assert(A == F);

                                          __m128i G =
  glm_i128_interleave(_mm_set_epi32(0, y, 0, x)); glm::uint64 Result[2];
  ...
  /*
                  {
                          ...
                          for(std::size_t i = 0; i < Data.size(); ++i)
                                  Data[i] = loopBitfieldInterleave(Param[i].x,
     Param[i].y);
  ...
  */
...
#if (GLM_ARCH & GLM_ARCH_SSE2_BIT && !(GLM_COMPILER & GLM_COMPILER_GCC))
  {
    ...
    for (std::size_t i = 0; i < SimdParam.size(); ++i)
      SimdParam[i] = _mm_set_epi32(
          static_cast<int>(i % static_cast<std::size_t>(x_max)), 0,
          static_cast<int>(i / static_cast<std::size_t>(y_max)), 0);
...
GLM_FUNC_QUALIFIER glm::uint16 bitfieldInterleave_u8vec2(glm::uint8 x,
                                                         glm::uint8 y) {
...
GLM_FUNC_QUALIFIER glm::u8vec2 bitfieldDeinterleave_u8vec2(
    glm::uint16 InterleavedBitfield) {
...
GLM_FUNC_QUALIFIER glm::uint32 bitfieldInterleave_u8vec4(glm::uint8 x,
                                                         glm::uint8 y,
                                                         glm::uint8 z,
                                                         glm::uint8 w) {
  glm::uint64 Result = (glm::uint64(w) << 48) | (glm::uint64(z) << 32) |
                       (glm::uint64(y) << 16) | glm::uint64(x);
  ...
  const glm::uint32 a =
      static_cast<glm::uint32>((Result & 0x000000000000FFFF) >> (0 - 0));
  const glm::uint32 b =
      static_cast<glm::uint32>((Result & 0x00000000FFFF0000) >> (16 - 3));
  const glm::uint32 c =
      static_cast<glm::uint32>((Result & 0x0000FFFF00000000) >> (32 - 6));
  const glm::uint32 d =
      static_cast<glm::uint32>((Result & 0xFFFF000000000000) >> (48 - 12));

  return a | b | c | d;
}

GLM_FUNC_QUALIFIER glm::u8vec4 bitfieldDeinterleave_u8vec4(
    glm::uint32 InterleavedBitfield) {
  ...
  return glm::u8vec4((Result >> 0) & 0x000000000000FFFFull,
                     (Result >> 16) & 0x00000000FFFF0000ull,
                     (Result >> 32) & 0x0000FFFF00000000ull,
                     (Result >> 48) & 0xFFFF000000000000ull);
}

GLM_FUNC_QUALIFIER glm::uint32 bitfieldInterleave_u16vec2(glm::uint16 x,
                                                          glm::uint16 y) {
  glm::uint64 Result = (glm::uint64(y) << 32) | glm::uint64(x);
  Result             = ((Result << 8) | Result) &
           static_cast<glm::uint32>(0x00FF00FF00FF00FFull);
  Result = ((Result << 4) | Result) &
           static_cast<glm::uint32>(0x0F0F0F0F0F0F0F0Full);
  Result = ((Result << 2) | Result) &
           static_cast<glm::uint32>(0x3333333333333333ull);
  Result = ((Result << 1) | Result) &
           static_cast<glm::uint32>(0x5555555555555555ull);
  return static_cast<glm::uint32>((Result & 0x00000000FFFFFFFFull) |
                                  (Result >> 31));
}

GLM_FUNC_QUALIFIER glm::u16vec2 bitfieldDeinterleave_u16vec2(
    glm::uint32 InterleavedBitfield) {
  glm::uint64 Result(InterleavedBitfield);
...
int test() {
  ...
  for (glm::size_t j = 0; j < 256; ++j)
    for (glm::size_t i = 0; i < 256; ++i) {
      glm::uint32 A = bitfieldInterleave_u8vec4(glm::uint8(i), glm::uint8(j),
                                                glm::uint8(i), glm::uint8(j));
      glm::uint32 B = glm::bitfieldInterleave(glm::uint8(i), glm::uint8(j),
                                              glm::uint8(i), glm::uint8(j));
      Error += A == B ? 0 : 1;
      /*
  ...
  for (glm::size_t j = 0; j < 256; ++j)
    for (glm::size_t i = 0; i < 256; ++i) {
      glm::uint32 A =
          bitfieldInterleave_u16vec2(glm::uint16(i), glm::uint16(j));
    ...
    for (glm::size_t j = 0; j < 256; ++j)
      for (glm::size_t i = 0; i < 256; ++i)
        Error += Result[j * 256 + i] ==
                         glm::bitfieldInterleave(glm::uint8(i), glm::uint8(j))
                     ? 0
                     : 1;
...
int perf_new_u8vec2(std::vector<glm::uint16>& Result) {
  ...
  for (glm::size_t k = 0; k < 10000; ++k)
    for (glm::size_t j = 0; j < 256; ++j)
      for (glm::size_t i = 0; i < 256; ++i)
        Error += Result[j * 256 + i] ==
                         bitfieldInterleave_u8vec2(glm::uint8(i), glm::uint8(j))
                     ? 0
                     : 1;
...
int perf_new_u8vec4(std::vector<glm::uint32>& Result) {
  ...
  for (glm::size_t k = 0; k < 10000; ++k)
    for (glm::size_t j = 0; j < 256; ++j)
      for (glm::size_t i = 0; i < 256; ++i)
        Error += Result[j * 256 + i] ==
                         bitfieldInterleave_u8vec4(glm::uint8(i), glm::uint8(j),
                                                   glm::uint8(i), glm::uint8(j))
                     ? 0
                     : 1;
...
int perf_old_u16vec2(std::vector<glm::uint32>& Result) {
  ...
  for (glm::size_t k = 0; k < 10000; ++k)
    for (glm::size_t j = 0; j < 256; ++j)
      for (glm::size_t i = 0; i < 256; ++i)
        Error += Result[j * 256 + i] ==
                         glm::bitfieldInterleave(glm::uint16(i), glm::uint16(j))
                     ? 0
                     : 1;
...
int perf_new_u16vec2(std::vector<glm::uint32>& Result) {
  ...
  for (glm::size_t k = 0; k < 10000; ++k)
    for (glm::size_t j = 0; j < 256; ++j)
      for (glm::size_t i = 0; i < 256; ++i)
        Error += Result[j * 256 + i] == bitfieldInterleave_u16vec2(
                                            glm::uint16(i), glm::uint16(j))
                     ? 0
                     : 1;
...
int perf() {
  ...
  for (glm::size_t j = 0; j < 256; ++j)
    for (glm::size_t i = 0; i < 256; ++i)
      Result_u8vec4[j * 256 + i] = glm::bitfieldInterleave(
          glm::uint8(i), glm::uint8(j), glm::uint8(i), glm::uint8(j));
