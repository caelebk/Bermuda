...
static_assert(sizeof(glm::lowp_u8vec1) == 1,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::mediump_u8vec1) == 1,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::highp_u8vec1) == 1,
              "uint8 size isn't 1 byte on this platform");

static_assert(sizeof(glm::lowp_u16vec1) == 2,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::mediump_u16vec1) == 2,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::highp_u16vec1) == 2,
              "uint16 size isn't 2 bytes on this platform");

static_assert(sizeof(glm::lowp_u32vec1) == 4,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::mediump_u32vec1) == 4,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::highp_u32vec1) == 4,
              "uint32 size isn't 4 bytes on this platform");

static_assert(sizeof(glm::lowp_u64vec1) == 8,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::mediump_u64vec1) == 8,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::highp_u64vec1) == 8,
              "uint64 size isn't 8 bytes on this platform");

static_assert(sizeof(glm::lowp_u8vec2) == 2,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::mediump_u8vec2) == 2,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::highp_u8vec2) == 2,
              "uint8 size isn't 1 byte on this platform");

static_assert(sizeof(glm::lowp_u16vec2) == 4,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::mediump_u16vec2) == 4,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::highp_u16vec2) == 4,
              "uint16 size isn't 2 bytes on this platform");

static_assert(sizeof(glm::lowp_u32vec2) == 8,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::mediump_u32vec2) == 8,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::highp_u32vec2) == 8,
              "uint32 size isn't 4 bytes on this platform");

static_assert(sizeof(glm::lowp_u64vec2) == 16,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::mediump_u64vec2) == 16,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::highp_u64vec2) == 16,
              "uint64 size isn't 8 bytes on this platform");

static_assert(sizeof(glm::lowp_u8vec3) == 3,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::mediump_u8vec3) == 3,
              "uint8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::highp_u8vec3) == 3,
              "uint8 size isn't 1 byte on this platform");

static_assert(sizeof(glm::lowp_u16vec3) == 6,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::mediump_u16vec3) == 6,
              "uint16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::highp_u16vec3) == 6,
              "uint16 size isn't 2 bytes on this platform");

static_assert(sizeof(glm::lowp_u32vec3) == 12,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::mediump_u32vec3) == 12,
              "uint32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::highp_u32vec3) == 12,
              "uint32 size isn't 4 bytes on this platform");

static_assert(sizeof(glm::lowp_u64vec3) == 24,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::mediump_u64vec3) == 24,
              "uint64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::highp_u64vec3) == 24,
              "uint64 size isn't 8 bytes on this platform");

static_assert(sizeof(glm::lowp_u8vec4) == 4,
              "int8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::mediump_u8vec4) == 4,
              "int8 size isn't 1 byte on this platform");
static_assert(sizeof(glm::highp_u8vec4) == 4,
              "int8 size isn't 1 byte on this platform");

static_assert(sizeof(glm::lowp_u16vec4) == 8,
              "int16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::mediump_u16vec4) == 8,
              "int16 size isn't 2 bytes on this platform");
static_assert(sizeof(glm::highp_u16vec4) == 8,
              "int16 size isn't 2 bytes on this platform");

static_assert(sizeof(glm::lowp_u32vec4) == 16,
              "int32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::mediump_u32vec4) == 16,
              "int32 size isn't 4 bytes on this platform");
static_assert(sizeof(glm::highp_u32vec4) == 16,
              "int32 size isn't 4 bytes on this platform");

static_assert(sizeof(glm::lowp_u64vec4) == 32,
              "int64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::mediump_u64vec4) == 32,
              "int64 size isn't 8 bytes on this platform");
static_assert(sizeof(glm::highp_u64vec4) == 32,
              "int64 size isn't 8 bytes on this platform");
...
static int test_fvec_precision() {
...

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f32vec2(v2), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec2(v3), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec2(v4), glm::epsilon<float>()))
                 ? 0
                 : 1;
  }

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f32vec3(v2), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec3(v3), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec3(v4), glm::epsilon<float>()))
                 ? 0
                 : 1;
  }

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f32vec4(v2), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec4(v3), glm::epsilon<float>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f32vec4(v4), glm::epsilon<float>()))
                 ? 0
                 : 1;
  }
...
static int test_dvec_precision() {
...

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f64vec2(v2), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec2(v3), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec2(v4), glm::epsilon<double>()))
                 ? 0
                 : 1;
  }

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f64vec3(v2), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec3(v3), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec3(v4), glm::epsilon<double>()))
                 ? 0
                 : 1;
  }

  {
    ...
    Error += glm::all(glm::equal(v1, glm::f64vec4(v2), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec4(v3), glm::epsilon<double>()))
                 ? 0
                 : 1;
    Error += glm::all(glm::equal(v1, glm::f64vec4(v4), glm::epsilon<double>()))
                 ? 0
                 : 1;
  }
