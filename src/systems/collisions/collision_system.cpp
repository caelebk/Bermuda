...
void CollisionSystem::collision_resolution_debug_info(Entity entity, Entity entity_other) {
  printf("Entity:\n");
...
void CollisionSystem::routePlayerProjCollisions(Entity player_proj, Entity other) {
  if (registry.deadlys.has(other)) {
...
void CollisionSystem::routeConsumableCollisions(Entity consumable, Entity other) {
  if (registry.players.has(other)) {
    resolvePlayerConsumableCollision(other, consumable);
  }
}

void CollisionSystem::routeInteractableCollisions(Entity interactable, Entity other) {
  if (registry.players.has(other)) {
...
void CollisionSystem::resolvePlayerInteractableCollision(Entity player, Entity interactable) {
  if (registry.deathTimers.has(player)) {
    return;
  }
  // TODO: add more affects M2+
...
void CollisionSystem::resolveEnemyPlayerProjCollision(Entity enemy, Entity player_proj) {
    PlayerProjectile& player_projectile = registry.playerProjectiles.get(player_proj);
...
void CollisionSystem::resolveWallPlayerProjCollision(Entity wall, Entity player_proj) {
...
void CollisionSystem::resolveWallPlayerCollision(Entity wall, Entity player) {
    ...
    //We need to find the smallest overlap horizontally and verticallly to determine where the overlap happened.
    ...
    //If the overlap in the X direction is smaller, it means that the collision occured here. Vice versa for overlap in Y direction.
