 ...
 *
 * @param entity
 * @param affector - affector's amount is positive if damaging, negative if refilling
 */
...
void checkAndRenderOxygen(Entity &entity, Oxygen &oxygen, float amount) {
    ...
    // modify oxygen bar
    if (registry.positions.has(oxygen.oxygenBar) && registry.positions.has(oxygen.backgroundBar))
    {
        auto &oxygenBarPos = registry.positions.get(oxygen.oxygenBar);
        bool isPlayer = registry.players.has(entity);
        float scaledAmount = (isPlayer) ? (amount / oxygen.capacity) * oxygenBarPos.originalScale.y : (amount / oxygen.capacity) * oxygenBarPos.originalScale.x;
        float &scale = (isPlayer) ? oxygenBarPos.scale.y : oxygenBarPos.scale.x;
        float &position = (isPlayer) ? oxygenBarPos.position.y : oxygenBarPos.position.x;
...
/**
 * @brief checks if player oxygen is low and adds/removes lowOxygen component accordingly
 */
void checkOxygenLevel(Entity &entity) {
    if (registry.oxygen.has(entity) && registry.players.has(entity)) { // TODO: REMOVE second cond if we want
        auto &oxygen = registry.oxygen.get(entity);                    // enemy health to change colours
...
/**
 * @brief update the position values of an enemy's health bar depending on enemy position
 */
void updateHealthBarAndEnemyPos(Entity &enemy) {
    Position &enemyPos = registry.positions.get(enemy);
    Oxygen &enemyOxygen = registry.oxygen.get(enemy);                        // TODO: ensure no unexpected behaviour once collision handling is fully implemented
    
    Position &oxygenBarPos = registry.positions.get(enemyOxygen.oxygenBar);
    Position &backgroundBarPos = registry.positions.get(enemyOxygen.backgroundBar);
    
    oxygenBarPos.position = enemyPos.position - vec2(0.f, enemyPos.scale.y / 2 + ENEMY_O2_BAR_GAP);
    backgroundBarPos.position = enemyPos.position - vec2(0.f, enemyPos.scale.y / 2 + ENEMY_O2_BAR_GAP);
}