...
void level_builder_demo() {
  ...
  // You can create a room or hallway and start building its boundaries by
  // 'drawing' it as a continuous line. Each unit here is a magnitude in object
  // space.
  ...
  // Doors can be added with the same chaining method, with the exception that
  // you don't have to specify a direction for them, since they're automatically
  // inferred by the API. Give each door an id so that we can connect them
  // later. Door ids are scoped to each room, not each level!
  level_builder.room("room_1_with_doors")
      .up(6)
      .right(6)
      .down(6)
      .left(2)
  ...
  // Hallways are built in exactly the same way. You can change the direction of
  // the builder's 'cursor' by calling a directional function without a
  // magnitude.
  level_builder.hallway("hallway_1")
      .up(12)
      .right()
      .door("n", 2)
      .down(12)
  ...
  // You can connect rooms and hallways together. Make sure to specify which
  // doors lead where.
  level_builder.room("room_2")
      .up(6)
      .right(2)
      .door("n", 2)
      .right(2)
  ...
  // And yes, this works on any non-square rectilinear space as well (rooms or
  // hallways).
  level_builder.hallway("complex")
      .up(10)
      .right(4)
      .door("n", 2)
      .right(4)
