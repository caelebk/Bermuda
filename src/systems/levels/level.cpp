...
void LevelBuilder::activate_boundary(Entity& boundary) {
  ...
  vec2 position = vec2(boundary_pos_x + ROOM_ORIGIN_POS.x,
                       boundary_pos_y + ROOM_ORIGIN_POS.y);
...
void LevelBuilder::spawn_miniboss() {
  execute_config_rand(
      CRAB_MINIBOSS, room(current_room_id),
      renderer);  // Replace "LVL_1_RAND_POS" with the macro for miniboss.
...
void LevelBuilder::activate_room(std::string room_id) {
  ...
  if (has_entered_room(current_room_id)) {
    // Retrieve all the saved entities and respawn them as they were
    while (saved_entities[current_room_id].size() > 0) {
      EntitySave es = saved_entities[current_room_id].back();
      es.respawn(renderer);
      saved_entities[current_room_id].pop_back();
    }
  } else if (boss_rooms.count(current_room_id)) {
...
void LevelBuilder::deactivate_room() {
  ...
  saved_entities[current_room_id].clear();

  for (auto& boundary :
       registry.spaces.get(room(current_room_id).entity).boundaries) {
  ...
  while (registry.deadlys.entities.size() > 0) {
    ...
    saved_entities[current_room_id].push_back(EntitySave(e));
...
void LevelBuilder::build_wall_with_random_doors(
    RoomBuilder& room, Direction direction,
...
void LevelBuilder::randomize_room_shapes(
  ...
  for (const auto& pair : adjacency_list) {
    ...
    build_wall_with_random_doors(
        current_room, Direction::WEST, directed_adjacencies, MAX_Y_UNITS, Y_1U,
    ...
    // north
    build_wall_with_random_doors(
        current_room, Direction::SOUTH, directed_adjacencies, MAX_X_UNITS, X_1U,
    ...
    // east
    build_wall_with_random_doors(
        current_room, Direction::EAST, directed_adjacencies, MAX_Y_UNITS, Y_1U,
    ...
    // south
    build_wall_with_random_doors(
        current_room, Direction::NORTH, directed_adjacencies, MAX_X_UNITS, X_1U,
...
// planar.
void LevelBuilder::generate_random_level(std::vector<int> rooms,
  ...
  std::unordered_map<int, std::set<int>> random_graph =
      generate_random_graph(rooms, densities);
