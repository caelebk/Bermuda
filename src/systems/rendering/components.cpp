...
#pragma warning(disable : 4996)
  ...
  while (1) {
    ...
    if (strcmp(lineHeader, "v") == 0) {
      ...
      {
        // Note first vertex index is already consumed by the first fscanf call
        // (match ==1) since it aborts on the first error
  ...
  if (abs(max_position.z - min_position.z) < 0.001)
    max_position.z =
        min_position.z +
        1;  // don't scale z direction when everythin is on one plane
  ...
  for (ColoredVertex& pos : out_vertices)
    pos.position =
        ((pos.position - min_position) / size3d) - vec3(0.5f, 0.5f, 0.5f);
